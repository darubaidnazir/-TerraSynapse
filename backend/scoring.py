...
def classify(index_name: str, value: float, crop_type: str, phenology_multiplier: float):
    db = SessionLocal()
    try:
...
def compute_health_score(indices: dict, crop_type: str = "default", planting_date: datetime = None):
    db = SessionLocal()
    try:
        score_sum = 0
        weight_sum = 0
        ...
        for index_name, data in indices.items():
            if index_name in ["ndvi", "savi", "evi", "ndmi", "ndwi"]:
                val = data.get("mean")
                if val is not None:
                    label, norm_score, is_default = classify(index_name, val, crop_type, phenology_multiplier)
